...
print("=" * 60)
print("HR ATTENDANCE — FULL FLOW TEST (CLEAN SLATE)")
print("=" * 60)

# Clean up any leftover test records for today
att, _ = req(f"{BASE}/api/hr-manager/attendance?startDate=2026-09-22&endDate=2026-09-22", "GET", hr_t)
for r in att:
    if r.get("employeeId") == "emp-001-demis-bekele":
        print(f"  Cleaning up old record: {r['id']}")
        req(f"{BASE}/api/hr-manager/attendance/{r['id']}", "DELETE", hr_t)
...
check("1. POST create Present", created, sc, 201)
...
if rid:
    ...
    hr_r2, sc4 = req(f"{BASE}/api/hr-manager/attendance/{rid}/hr-status", "PATCH",
                     {"hrStatus": "Approved", "hrNote": "Within grace period"}, hr_t)
    rd2 = hr_r2.get("record", {})
    check("4. PATCH hrStatus→Approved", rd2, sc4, 200)

    # 5. PATCH hrStatus → Absent
    ...
    # 6. Employee portal sees the HR override
    punch, sc6 = req(f"{BASE}/api/employer/punch", "GET", emp_t)
    check("6. Employee GET /punch", punch, sc6, 200)
    print(f"     → hrStatus: {punch.get('hrStatus')}")
    print(f"     → hrNote:    {punch.get('hrNote')}")
